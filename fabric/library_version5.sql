drop table if exists fabric.lib_master6;

select *
into fabric.lib_master6
from analysis.imls_lib_2012;
----17586 records

delete from fabric.lib_master6
...
alter table fabric.lib_master6
...
from new_values
where lib_master6.fscskey = new_values.fscskey;
...
--CAI
select *
from fabric.lib_master6, fabric.imls_cai
where lib_master6.libid = imls_cai.imlsid;
...
from new_values
where lib_master6.libid = new_values.imlsid;
...
--KANSAS
select *
from fabric.lib_master6, fabric.ks_lib
where lib_master6.libname = upper(ks_lib.library_name)
	and stabr = 'KS';
...
from new_values
where lib_master6.me_fscskey = new_values.fscs_id;

--MISSOURI
select *
from fabric.lib_master6, fabric.mo_lib
where lib_master6.libname = upper(mo_lib.site_name)
	and stabr = 'MO';
...
from new_values
where lib_master6.libname = upper(new_values.site_name)
	and stabr = 'MO';

--VERMONT
select *
from fabric.lib_master6, fabric.vt_lib
where lib_master6.libid = vt_lib.library_id
	and stabr = 'VT';
...
from new_values
where lib_master6.libid = new_values.library_id
	and stabr = 'VT';

--OHIO
select *
from fabric.lib_master6, fabric.oh_lib
where lib_master6.fscskey = oh_lib.fscs
	and lib_master6.libname = oh_lib.library_name;
...
from new_values
where lib_master6.fscskey = new_values.fscs
	and lib_master6.libname = new_values.library_name;
...
with new_values as(
select libid, coalesce(alliance,0) + coalesce(armstrong,0) + coalesce(ayrshire,0) + coalesce(blue_ridge,0) + coalesce(bolivar,0) + coalesce(bucks,0)
	 + coalesce(c_spire,0) + coalesce(cai,0) + coalesce(carnegie,0) + coalesce(cascade_com,0) + coalesce(central_mississippi,0)
	 + coalesce(clear_lake,0) + coalesce(columbus_lowndes,0) + coalesce(com_net,0) + coalesce(corinth,0) 
	 + coalesce(copiah_jefferson,0) + coalesce(covington,0) + coalesce (ctc,0) + coalesce(cunningham,0) 
	 ...
	 + coalesce(fibercomm,0) + coalesce(fidelity,0) + coalesce(fontana,0) + coalesce(gervais_datavision,0) + coalesce(golden_belt,0)
	 + coalesce(grantsburg,0) + coalesce(h_and_b,0) + coalesce(harriette,0) + coalesce(harrison,0) + coalesce(heart_iowa,0) 
	 + coalesce(iowa,0)
	 + coalesce(jackson_george,0) + coalesce(kansas,0) + coalesce(kemper_newton,0) + coalesce(lamar,0) + coalesce(lee_itawamba,0)
	 + coalesce(madison,0)  
	 + coalesce(maine,0) + coalesce(manawa,0) 
	 + coalesce(marks_quitman,0) + coalesce(mccormack,0) + coalesce(mid_mississippi,0) + coalesce(missouri,0) 
	 + coalesce(meridian_lauderdale,0) + coalesce(nemr,0) + coalesce(nextech,0)
	 + coalesce(northwest,0) + coalesce(ohio,0) + coalesce(panora,0) + coalesce(paul_bunyan,0) + coalesce(pearl_river,0) + coalesce(peoples_rural,0)
	 + coalesce(peoples_telecom,0) + coalesce(pike,0) + coalesce(pioneer,0) + coalesce(premier,0) + coalesce(range_telephone,0)
	 + coalesce(rt_comm,0) + coalesce(scmtc,0) + coalesce(siskiyou,0) + coalesce(smrl,0) + coalesce(srtc,0) + coalesce(stayton,0) + coalesce(sunflower,0)
	 ...
	 + coalesce(west_carolina_tel,0) + coalesce(westel,0) + coalesce(wilkinson,0) + coalesce(wilson,0) + coalesce(xit,0) 
...
from new_values
where lib_master6.libid = new_values.libid;
...
select fscskey AS fscs_key, system_name, libid AS library_id, libname AS library_name, c_out_ty AS library_type, geom, visits, 
	...
	alliance, armstrong, ayrshire, blue_ridge, bolivar, bucks, c_spire, cai, carnegie, cascade_com, central_mississippi, clear_lake, columbus_lowndes,
	com_net, copiah_jefferson, corinth, covington, ctc, cunningham, dc, dixie, dobson, dubois_telephone, dumont, einetwork, endeavor, fibercomm, fidelity, fontana,
	gervais_datavision, golden_belt, grantsburg, h_and_b, harriette, harrison, heart_iowa, iowa, jackson_george, kansas, kemper_newton, lamar, lee_itawamba,
	madison, maine, manawa, marks_quitman, mccormack, meridian_lauderdale, mid_mississippi, missouri, nemr, nextech, northwest, ohio, panora, paul_bunyan,
	pearl_river, peoples_rural, peoples_telecom, pike, pioneer, premier, range_telephone, rt_comm, scmtc, siskiyou, smrl, srtc, stayton,
	sunflower, tallahatchie, toledotel, tombigbee, triangle, united, us_connect, van_horne, vermont, wabash, waynesboro_wayne,
	webster_calhoun, west_carolina_tel, westel, wilkinson, wilson, xit, yalobusha, yazoo
from fabric.lib_master6
);
