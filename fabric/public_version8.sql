--CREATE MASTER TABLE
drop table if exists fabric.master9;

create table fabric.master9 as
...
select fiber, count(*)
from fabric.master9, analysis.cai_dec2013
where cai_dec2013.caiid = master9.ncessch
	and lstate = 'AK'
group by fiber;

alter table fabric.master9
...
update fabric.master9
	set cai1 = new_values.cai_fiber
	from new_values
	where new_values.caiid=master9.ncessch;

select seasch, caiid, lstate, statecode, fiber
from fabric.master9, analysis.cai_dec2013
where cai_dec2013.caiid = master9.seasch
...
update fabric.master9
	...
	from new_values
	where new_values.caiid=master9.seasch
		and statecode=lstate;

select stid, caiid, lstate, statecode fiber
from fabric.master9, analysis.cai_dec2013
where cai_dec2013.caiid = master9.stid
	and statecode = lstate;
...
update fabric.master9
	...
	from new_values
	where new_values.caiid=master9.stid
		and statecode = lstate;
...
select st_cd, cds_code, fiber
	from fabric.master9, fabric.cenic
	where master9.st_cd = cds_code
	and lstate = 'CA';
...
update fabric.master9
	...
	from new_values
	where new_values.cds_code=master9.st_cd
		and lstate = 'CA';
...
update fabric.master9
	...
	from new_values
	where master9.scd_fl=new_values.school_code
		and lstate = 'FL';
...
update fabric.master9
	...
	from new_values
	where master9.seasch=new_values.wv_school_id
		and lstate = 'WV';
...
update fabric.master9
	...
	from new_values
	where master9.ncessch=new_values.nces_id
		and lstate = 'NM';
...
update fabric.master9
	...
	from new_values
	where master9.schnam = upper(new_values.school_name)
		and master9.leanm = upper(new_values.district_name)
		and lstate = 'MT';
...
select sunesys.rev_appname, leanm, appstate, lstate
	from fabric.sunesys, fabric.master9
...
update fabric.master9
	...
	from new_values
	where master9.schnam = upper(new_values.building_name)
	and master9.lcity = upper(new_values.building_city)
	and master9.lstate = 'OH';
...
from new_values
where master9.seasch = new_values.school_code;
...
update fabric.master9
	...
	from new_values
	where new_values.nces_id=master9.ncessch;
...
--MISSISSIPPI
alter table fabric.master9
add column mississippi int;

with new_values as(
select district, school, fiber
from fabric.ms_ind
)
update fabric.master9
set mississippi = fiber
from new_values
...
--MINNESOTA
alter table fabric.master9
drop column if exists minnesota_nw;
alter table fabric.master9
add column minnesota_nw int;

with new_values as(
select nces_sch_id, fiber
from fabric.mn_ind
)
update fabric.master9
set minnesota_nw = new_values.fiber
from new_values
where ncessch = nces_sch_id;
...
update fabric.master9
	set mte = 9
	where ncessch in ('160216000857','040954000978');
update fabric.master9
	set mte = -9
	where ncessch = '160072000143' or (leaid = '0409540' and mte IS NULL);
...
update fabric.master9
set h_and_b = 9
where ncessch in ('200465000912','200465000914','200465000913','200034902044','200034902042','200034902043','20034901970',
	'20034902028','20034901992');
...
add column telalaska int,
add column blackfoot int,
add column logan int,
add column united_com int,
add column eastland int,
add column dakota int,
add column tussey int,
add column chariton_valley int,
add column twin_valley int,
add column west_liberty int,
add column eureka int,
add column matanuska_susitna int,
add column comm1_net int,
add column downingtown int,
add column oran int,
add column bainbridge_island int,
add column cross_tel int,
add column cimarron int,
add column pottawatomie int,
add column mbo int,
...
update fabric.master9
set telalaska = 9
where ncessch = '20039000437';

update fabric.master9
set logan = 9
where ncessch in ('210354001458','210354001542','210354001543','210354001544','210354001545');
update fabric.master9
set logan = 0
where ncessch = '210354001546';

update fabric.master9
set blackfoot = 9
where ncessch in ('300186000003','300186001007','300186001004','302082000622','302082000896','302085000623',
	'300611000175','300611000994','300611000174','302493000714','302493000715','302493001009');

update fabric.master9
set united_com = 9
where ncessch in ('470018000039','470267000950','470267000482','470018002218','470453001811','470453002097',
	'470453001802','470129000401');
...
update fabric.master9
set dakota = 9
where ncessch in ('380005300088','380005300087','380004300231','380004300230','380002100310','380002100311','380002100312','380002100825'
	,'380002100313','380002100314','380002100315','388068000720','381026000330','381026000329','381266000419','381266000418'
	,'381323000459','381323000458');

update fabric.master9
set tussey = 9
where ncessch = '422397000748';
update fabric.master9
set tussey = -9
where ncessch in ('422397007360','422397000753','422397000752','422397000751');

update fabric.master9
set chariton_valley = 9
where ncessch in ('290348000029','290348000030','290498000062','290498000063','290609000146','290609000147','291350000573',
	'291350000574','292188001220','292188001221','292752001683','292752001685','293186002230','293186002232');
update fabric.master9
set chariton_valley = -9
where ncessch in ('290561000106','290561000107','293186002514');

update fabric.master9
set twin_valley = 9
where ncessch in ('200651000649','200651000650','200495000102','200495000099','200495001612','200489000824','200489000823',
	'200489000821','200489000822','200651000647','200651000648','200957000161','200957000163','200396000164','200396000165',
	'200489000828','200489000829','200789001268','200396000166','200396000167','201110000819','201110000820','201098000857',
	'201098000856','201098000855');


update fabric.master9
set west_liberty = 9
where ncessch in ('193075001771','193075001774','193075001773','193099001408','193099002109','193099001805','193099001804');
...
update fabric.master9
set matanuska_susitna = 9
where ncessch in ('020051000311','020051000452','020051000727','020051000744','020051000720','020051000724','020051000731',
	'020051000711','020051000719','020051000514','020051000585','020051000011','020051000586','020051000450','020051000226',
	'020051000733','020051000443','020051000741','020051000224','020051000734','020051000407','020051000416','020051000312',
	'020051000227','020051000544','020051000565','020051000480','020051000230','020051000414','020051000725','020051000232',
	'020051000233','020051000723','020051000547','020051000444','020051000722','020051000469','020051000425','020051000237',
	'020051000545','020051000721');
update fabric.master9
set matanuska_susitna = -9
where ncessch in ('020051000726','020051000740','020051000225');
update fabric.master9
set matanuska_susitna = 0
where ncessch = '020051000747';

update fabric.master9
set comm1_net = 9
where ncessch in ('190543000172','190543000928','190543000171','191233000753','191233000754','191233000752','190819000407');

update fabric.master9
set downingtown = 9
where ncessch in ('420771000706','420771001387','420771007401','420771001391','420771009949','420771001431','420771000308',
	'420771005011','420771006476','420771009948','420771006621','420771006477','420771000307','420771001432','420771006843');
...
update fabric.master9
set bainbridge_island = 9
where leaid = '5300330';

update fabric.master9
set cross_tel = 9
where ncessch in ('403165001750','403165029708','403213001779','403213029707','401290001982','401290002202','401290002294',
	'401635000757','401635000758','401941000915','401941000916','401662000775','401662000306','402541029745','402541001353',
	'400735000290','400735000571','400735001859','400639002753','402484001306','402484001307','402865001502','402865001503',
	'402865001504','403270001806');

update fabric.master9
set cimarron = 9
where ncessch in ('401884002716','401884000894','401884000893','401884000895','401884002301','401647000766','402517001318',
	'402517001319','401575000741');

update fabric.master9
set pottawatomie = 9
where ncessch in ('401050000469','401050000468','400513000179','400513000180','402694001426','402694001427');

update fabric.master9
set mbo = 9
where ncessch in ('400804000332','400804000335','400804000333','400804000331','402286001217','402286029655','401929000911','401929000912');

update fabric.master9
set union_tel = 9
where ncessch in ('551182001550','551182001548','551182002620','550024000023','550024000024','550024000025','551644002147');

update fabric.master9
set winthrop = 9
where ncessch = '271258 000044';

update fabric.master9
set lemonweir_valley = 9
where ncessch in ('551491001933','551056001381','551056001382','550003502633','550885002574','550885002863','550885001025',
	'550885001026','550003602636','550885001027');

update fabric.master9
set la_valle = 9
where ncessch in ('551647000548','551647002156','551647002155','551266001662');
...
alter table fabric.master9
add column central_arkansas int, 
...
add column valley int,
add column vtx int;

update fabric.master9
set central_arkansas = 9
where leaid in ('0503240','0510980');

update fabric.master9
set madison_county = 9
where ncessch in ('050824000564','050824000563','050813000538','050813000662','050813000539','050813000537');

update fabric.master9
set wiggins = 9
where ncessch in ('080729001258','080729001257','080729006394','080255000143','080255000142','080672001158',
	'080672001157','080444000653','080444000652');

update fabric.master9
set cameron = 9
where ncessch in ('220039000289','220033000237','220006000025','220006000024','220018001507','220018000083','220129001071',
	'220183001413','220039000290','220039000291','220039000293','220033000258','220033000320');

update fabric.master9
set cameron = -9
where ncessch in ('482316002351','482316002350','482316007293');

update fabric.master9
set three_rivers = 9
where ncessch in ('300243000024','300243000997','300245000025','300342000047','300342000925','300342000943','300345000048',
	'300065400135','300065400604','300065401099','300514000872','300514001100','300519000136','300533000157','300627000182',
	'300632000183','300632001065','300719001060','300733000207','300733000208','300918000984','302106000631','300984000276',
	'300984000293','300993000277','301008000279','301008001000','301014000280','301218000332','301221000333','301227000334',
	'301386000432','301386001010','301389000433','302133000639','302133001011','302145000641','302499000717','302502000718',
	'302502000719','302505000720','302367000949','302373000689','302373001018','302385000690','302391000694','302394000695',
	'302394001083','302418000698','302420000699','302423000702','302658000749','302664000750','302664000933','302688000923');

update fabric.master9
set three_rivers = -9
where ncessch in ('301719000251','300514000129','300514000131','300514000132','300514000134','300514000812','300514000813',
	'300720000205','300732000210','301251000345','301304000883','301305000379','301218000948','301344000401','301344001076',
	'301347000402','301649000510','301653000512','301653000517','302688000756','302688000945');
...
update fabric.master9
set triangle = 9
where leaid in ('3014150','3013590','3013560','3004440','3004500','3000096','3000097','3022750','3028911','3003000','3006260',
...
update fabric.master9
set triangle = -9
where leaid in ('3012350','3027930','3013440','3023850','3013140','3018060','3018290','3018660','3021870','3021900','3028380',
	'3004890','3004920');
...
update fabric.master9
set valley = 9
where leaid in ('4839000','4826850','4838910','4844810','4829940','4836420','4837140')
	or ncessch in ('483774011381','483774004230');

update fabric.master9
set vtx = 9
where ncessch in ('481710001482','481710001484','481710001483','483936004479','483936006324','481158000632','481158000629',
	'481158000631','481158000630','483372003762','483372006308','483372003764','483372003763','482055002064','482055002066',
	'482055002065') or leaid in ('4828620','4835760','4807530','4842690','4824960','4813710','4836540');
...
with new_values as(
select ncessch, coalesce(advanced,0) + coalesce(alabama,0) + coalesce(alenco_comm,0) + coalesce(allen,0) 
	+ coalesce(alliance,0) + coalesce(alpine,0) + coalesce(arlington,0) 
	...
	+ coalesce(butler_bremer,0) + coalesce(c_spire,0) + coalesce(cai,0) + coalesce(california,0) + coalesce(cameron,0)
	+ coalesce(chariton_valley,0) + coalesce(central_arkansas,0)
	...
	+ coalesce(farmers,0) + coalesce(fatbeam,0) + coalesce(ferndale,0)
	+ coalesce(fibercomm,0) + coalesce(fidelity,0) + coalesce(filer,0)
	+ coalesce(florida,0) + coalesce(fort_laboeuf,0) 
	...
	+ coalesce(lakeview,0) 
	+ coalesce(laporte,0) + coalesce (lebanon,0) + coalesce(lemonweir_valley,0) + coalesce(logan,0) 
	+ coalesce(madison_county,0) + coalesce(maine,0) + coalesce(manawa,0) + coalesce(manheim,0)
	...
	+ coalesce(mississippi,0) + coalesce(moniteau,0)
	+ coalesce(monon,0) + coalesce(montana,0) + coalesce(mtc,0) + coalesce(mte,0) + coalesce(navajo,0) + coalesce(nctc,0)
	+ coalesce(nemr,0)
	+ coalesce(new_mexico,0) + coalesce(newton,0) + coalesce(nextech,0) + coalesce(nogales,0) 
	+ coalesce(north_arkansas,0) + coalesce(north_carolina,0)
	...
	+ coalesce(pioneer,0) + coalesce(pmt,0) + coalesce(ponderosa,0) + coalesce(portage,0) + coalesce(pottawatomie,0)
	...
	+ coalesce(south_central,0) + coalesce(south_middleton,0) + coalesce(southern_kansas,0)
	+ coalesce(southwest_arkansas,0) + coalesce(southwest_texas,0) 
	...
	+ coalesce(telalaska,0) + coalesce(three_rivers,0) + coalesce(toledotel,0) + coalesce(totah_totel,0) 
	+ coalesce(tri_county,0) + coalesce(triangle,0)
	...
	+ coalesce(us_connect,0) + coalesce(valley,0) + coalesce(van_buren,0) + coalesce(van_horne,0) + coalesce(vtx,0)
	+ coalesce(wabash,0) + coalesce(waldron,0) 
	...
	+ coalesce(wiggins,0) + coalesce(wikstrom,0) + coalesce(wilson,0) + coalesce(winthrop,0) + coalesce(wittenberg,0)
...
from new_values
where master9.ncessch = new_values.ncessch;
...
alter table fabric.master9
	drop column if exists fiber_v7,
...
update fabric.master9
set fiber_v7 = new_values.fiber_map
from new_values
where master9.ncessch = new_values.ncessch;
...
select ncessch AS nces_id, schnam AS school_name, leaid AS lea_id, leanm AS lea_name, lcity AS city, lstate AS state, 
	ulocal AS locale, member AS enrollement, geom, fiber_map AS fiber_v7, fiber_v6, fiber_v5, fiber_v4, fiber_v3, fiber_v2, fiber_v1, score_map AS score,
	advanced, alabama, alenco_comm, allen, alliance, alpine, arlington, atc, bainbridge_island, bellwood_antis, bie, blackfoot, blue_mountain, 
	bps, butler_bremer, c_spire, cai, california, cameron, central_arkansas, chariton_valley, cimarron, citizens, clear_lake, com_net, comm1_net, cross_tel, ctc, 
	...
	endeavor, eureka, farmers, fatbeam, ferndale, fibercomm, fidelity, filer, florida, fort_laboeuf, franklin, ftc, galena, ganado, garden_valley, 
	...
	laporte, lebanon, lemonweir_valley, logan, madison_county, maine, manawa, manheim, marion, marne_elk, matanuska_susitna, mbo,
	mccormack, mckenzie, mcleod, middleburgh, midstate, mifflin, mississippi, moniteau, monon, montana, mtc, mte, navajo, nctc, nemr, 
	new_mexico, newton, nextech, nogales, north_arkansas, north_carolina, northampton, nu_telecom, octorara, ohio, oran, oregon_farmers, ortelco, otelco, 
	pa_cyber, palestine, panhandle, park_region, paul_bunyan, peoples_rural, peoples_telecom, phoenixville, pinnacle, pioneer, pmt, 
	ponderosa, portage, pottawatomie, premier, 
	...
	south_central, south_middleton, southern_kansas, southwest_arkansas, southwest_texas, spring_grove, spring_grove_comm, srtc, stayton, sunesys, 
	tca, tec, telalaska, three_rivers, toledotel, totah_totel, tri_county, triangle, tussey, twin_valley, union_tel, united, 
	united_com, upper_dauphin, us_connect, valley, van_buren, van_horne, vtx, wabash, waldron, wayne_highlands, weatherly, west_carolina_tel, 
	west_central, west_liberty, west_texas_rural, west_virginia, western_iowa, wiggins, wikstrom, wilson, winthrop, wittenberg, xit, 
	yadtel, yelcot
from fabric.master9
);
