drop table if exists fabric.lib_master9;

select *
into fabric.lib_master9
from analysis.imls_lib_2012;
----17586 records

ALTER TABLE fabric.lib_master9
 ADD COLUMN gid SERIAL PRIMARY KEY;

delete from fabric.lib_master9
where c_out_ty = 'BM' or c_out_ty = 'BS';

--Update matching key
update fabric.lib_master9
...
from new_values
where lib_master9.fscskey = new_values.fscskey;
...
--CAI
select *
from fabric.lib_master9, fabric.imls_cai
where lib_master9.libid = imls_cai.imlsid;
...
from new_values
where lib_master9.libid = new_values.imlsid;
...
select *
from fabric.lib_master9;

--KANSAS
select *
from fabric.lib_master9, fabric.ks_lib
where lib_master9.libname = upper(ks_lib.library_name)
	and stabr = 'KS';
...
from new_values
where lib_master9.me_fscskey = new_values.fscs_id;

--MISSOURI
select *
from fabric.lib_master9, fabric.mo_lib
where lib_master9.libname = upper(mo_lib.site_name)
	and stabr = 'MO';
...
from new_values
where lib_master9.libname = upper(new_values.site_name)
	and stabr = 'MO';

--VERMONT
select *
from fabric.lib_master9, fabric.vt_lib
where lib_master9.libid = vt_lib.library_id
	and stabr = 'VT';
...
from new_values
where lib_master9.libid = new_values.library_id
	and stabr = 'VT';

--OHIO
select *
from fabric.lib_master9, fabric.oh_lib
where lib_master9.fscskey = oh_lib.fscs
	and lib_master9.libname = oh_lib.library_name;
...
from new_values
where lib_master9.fscskey = new_values.fscs
	and lib_master9.libname = new_values.library_name;
...
update fabric.lib_master9
set vtx = 9
where libid in ('TX0583-002','TX0243-002','TX0040-002','TX0406-004','TX0406-003','TX0061-003','TX0117-002','TX0383-002');
...
select libid, coalesce(allegheny,0) + coalesce(alliance,0) + coalesce(armstrong,0) + coalesce(ayrshire,0) 
	...
	+ coalesce(bolivar,0) + coalesce(bucks,0) + coalesce(cameron,0)
	 ...
	 + coalesce(vermont,0) + coalesce(vtx,0) + coalesce(wabash,0) + coalesce(waynesboro_wayne,0) + coalesce(webster_calhoun,0)
...
from new_values
where lib_master9.libid = new_values.libid;
...
select fscskey AS fscs_key, system_name, libid AS library_id, libname AS library_name, c_out_ty AS library_type, geom, visits, 
	fiber_map AS fiber_v8, fiber_v7, fiber_v6, fiber_v5, fiber_v4, fiber_v3, fiber_v2, fiber_v1, score_map AS score, 
	...
	sunflower, tallahatchie, toledotel, tombigbee, triangle, twin_valley, union_tel, united, us_connect, van_horne, vermont, vtx, wabash, 
