--CREATE MASTER TABLE
drop table if exists fabric.master8;

create table fabric.master8 as
...
select fiber, count(*)
from fabric.master8, analysis.cai_dec2013
where cai_dec2013.caiid = master8.ncessch
	and lstate = 'AK'
group by fiber;
...
update fabric.master8
	...
	from new_values
	where new_values.caiid=master8.ncessch;

select seasch, caiid, lstate, statecode, fiber
from fabric.master8, analysis.cai_dec2013
where cai_dec2013.caiid = master8.seasch
...
update fabric.master8
	...
	from new_values
	where new_values.caiid=master8.seasch
		and statecode=lstate;

select stid, caiid, lstate, statecode fiber
from fabric.master8, analysis.cai_dec2013
where cai_dec2013.caiid = master8.stid
	and statecode = lstate;
...
update fabric.master8
	...
	from new_values
	where new_values.caiid=master8.stid
		and statecode = lstate;
...
select st_cd, cds_code, fiber
	from fabric.master8, fabric.cenic
	where master8.st_cd = cds_code
	and lstate = 'CA';
...
update fabric.master8
	...
	from new_values
	where new_values.cds_code=master8.st_cd
		and lstate = 'CA';
...
update fabric.master8
	...
	from new_values
	where master8.scd_fl=new_values.school_code
		and lstate = 'FL';
...
update fabric.master8
	...
	from new_values
	where master8.seasch=new_values.wv_school_id
		and lstate = 'WV';
...
update fabric.master8
	...
	from new_values
	where master8.ncessch=new_values.nces_id
		and lstate = 'NM';
...
update fabric.master8
	...
	from new_values
	where master8.schnam = upper(new_values.school_name)
		and master8.leanm = upper(new_values.district_name)
		and lstate = 'MT';
...
select sunesys.rev_appname, leanm, appstate, lstate
	from fabric.sunesys, fabric.master8
...
update fabric.master8
	...
	from new_values
	where master8.schnam = upper(new_values.building_name)
	and master8.lcity = upper(new_values.building_city)
	and master8.lstate = 'OH';
...
from new_values
where master8.seasch = new_values.school_code;
...
update fabric.master8
	...
	from new_values
	where new_values.nces_id=master8.ncessch;
...
from new_values
where master8.ncessch = new_values.ncessch;
