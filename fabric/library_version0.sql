...
--OHIO
select *
from fabric.lib_master, fabric.oh_lib
where lib_master.fscskey = oh_lib.fscs
	and lib_master.lib_name = oh_lib.library_name;
...
from new_values
where lib_master.fscskey = new_values.fscs
	and lib_master.lib_name = new_values.library_name;
...
with new_values as(
select lib_id, coalesce(cai,0) + coalesce(ks_lib,0) + coalesce(me_lib,0)
...
drop table if exists fabric.libmap_fiber_aug1;
create table fabric.libmap_fiber_aug1 as(
select lib_id, geom, block_fips, score_map, fiber_map
from fabric.lib_master
);

copy(select * from fabric.libmap_fiber_aug1) to '/Users/FCC/Documents/allison/E-rate analysis/Maps/library_map_fiber_aug1.csv' with delimiter '|' CSV header;
...
with new_values as(
select lib_id, coalesce(cai,0) + coalesce(ks_lib,0) + coalesce(me_lib,0) + coalesce(mo_lib,0) + coalesce(vt_lib,0) 
	+ coalesce(oh_lib,0)
	as row_score
	from fabric.lib_master
)
