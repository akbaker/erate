--CREATE MASTER TABLE
drop table if exists fabric.master6;

create table fabric.master6 as
...
select fiber, count(*)
from fabric.master6, analysis.cai_dec2013
where cai_dec2013.caiid = master6.ncessch
	and lstate = 'AK'
group by fiber;

alter table fabric.master6
...
update fabric.master6
	set cai1 = new_values.cai_fiber
	from new_values
	where new_values.caiid=master6.ncessch;

select seasch, caiid, lstate, statecode, fiber
from fabric.master6, analysis.cai_dec2013
where cai_dec2013.caiid = master6.seasch
...
update fabric.master6
	...
	from new_values
	where new_values.caiid=master6.seasch
		and statecode=lstate;

select stid, caiid, lstate, statecode fiber
from fabric.master6, analysis.cai_dec2013
where cai_dec2013.caiid = master6.stid
	and statecode = lstate;
...
update fabric.master6
	...
	from new_values
	where new_values.caiid=master6.stid
		and statecode = lstate;
...
select st_cd, cds_code, fiber
	from fabric.master6, fabric.cenic
	where master6.st_cd = cds_code
	and lstate = 'CA';
...
update fabric.master6
	...
	from new_values
	where new_values.cds_code=master6.st_cd
		and lstate = 'CA';
...
update fabric.master6
	...
	from new_values
	where master6.scd_fl=new_values.school_code
		and lstate = 'FL';
...
update fabric.master6
	...
	from new_values
	where master6.seasch=new_values.wv_school_id
		and lstate = 'WV';
...
update fabric.master6
	...
	from new_values
	where master6.ncessch=new_values.nces_id
		and lstate = 'NM';
...
update fabric.master6
	...
	from new_values
	where master6.schnam = upper(new_values.school_name)
		and master6.leanm = upper(new_values.district_name)
		and lstate = 'MT';
...
select sunesys.rev_appname, leanm, appstate, lstate
	from fabric.sunesys, fabric.master6
...
update fabric.master6
	...
	from new_values
	where master6.schnam = upper(new_values.building_name)
	and master6.lcity = upper(new_values.building_city)
	and master6.lstate = 'OH';
...
from new_values
where master6.seasch = new_values.school_code;
...
update fabric.master6
	...
	from new_values
	where new_values.nces_id=master6.ncessch;
...
from new_values
where master6.ncessch = new_values.ncessch;
